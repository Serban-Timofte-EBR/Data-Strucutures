...
void stergere_cb_moneda(NodLS** HT, unsigned char ht_size, char* moneda)
{
	for (unsigned char i = 0; i < ht_size; i++)
	{
		...
		while (temp != NULL)
		{
			if (strcmp(temp->data.moneda, moneda) == 0)
			{
				printf("Stergere %s %s\n", temp->data.iban, temp->data.moneda);
				if (prev == NULL) 
				{
					HT[i] = temp->next;
				}
...
int main()
{
	...
	for (unsigned char i = 0; i < HTABLE_SIZE; i++) {
		if (HTable[i] != NULL) {
			...
			while (temp != NULL)
			{
				printf("%s %s\n", temp->data.iban, temp->data.moneda, temp->data.moneda);
				temp = temp->next;
			}
		}
	}

	stergere_cb_moneda(HTable, HTABLE_SIZE, "RON");
